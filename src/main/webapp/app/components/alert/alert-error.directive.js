(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('emrexDemoApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-emrexDemoApp-error');
                var entityKey = httpResponse.headers('X-emrexDemoApp-params');
                if (errorHeader) {
